...
```{r include=FALSE}
episodes_count <- length(unique(season_data$no_overall))
...
avg <- round(mean(season_data$viewers), 2)
vwrs_start <- season_data[1,]$viewers
vwrs_end <- season_data[nrow(season_data),]$viewers
